 ...
 * Uses `X-API-Key` for authentication with the PowerDNS API.
...
export class ApiClient {
    ...
    /**
     * Makes an HTTP PATCH request to the specified endpoint with a JSON payload.
     *
     * @async
     * @function patch
     * @param {string} endpoint - The API endpoint to call (relative to the base URL).
     ...
     * @throws {Error} - If the HTTP request fails or the response is not successful.
     */
    public async patch(endpoint: string, body: any, headers: Record<string, string> = {}): Promise<any> {
     ...
     * This method handles low-level HTTP operations, including adding the `X-API-Key`
